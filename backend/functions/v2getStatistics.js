const getStatisticsDateRangeV2 = require('./v2getStatisticsDateRange');

async function getStatisticsV2(uid) {
  const date = new Date(new Date().toUTCString());
  const fy = getFinancialYear(date);
  const financialYearObj = await getStatisticsDateRangeV2(uid, fy.start, fy.end);
  ...
  const financialQuaterObj = await getStatisticsDateRangeV2(uid, fq.start, fq.end);
  ...
  const monthlyObj = await getStatisticsDateRangeV2(uid, start, end);
  ...
  const weeklyObj = await getStatisticsDateRangeV2(uid, firstDayOfWeek, lastDayOfWeek);
  ...
  const dailyObj = await getStatisticsDateRangeV2(uid, today, today);
...
module.exports = getStatisticsV2;
